...
> Answer:"ifelse()" worked because it is vectorized, so it can check each value in a vector and return a result for each element. "if...else" only works with a single TRUE/FALSE condition, so it does not work properly when the input is a vector.
