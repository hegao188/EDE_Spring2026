...
```{r data.summary, echo=TRUE, results='asis'}
#Compute mean PM2.5 concentration for each county, sorted by value
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_val = mean(`PM2.5`), .groups = "drop") %>%
  arrange(mean_val) %>%
  mutate(mean_val = round(mean_val, 2)) %>%
  rename(
    County = COUNTY,
    `$\\mu g/m^3$` = mean_val
  )
...
summary_Ozone <- epa_data %>%
  drop_na(Ozone) %>%
  group_by(COUNTY) %>%
  summarize(mean_val = mean(Ozone), .groups = "drop") %>%
  arrange(mean_val) %>% 
  mutate(mean_val = round(mean_val, 2)) %>%
  rename(
    County = COUNTY,
...
```{r ozone_box_2018, echo=FALSE, fig.align='left', fig.height=3.0, fig.cap='Ozone distribution by month (2018).'}
epa_data %>%
...
```{r ozone_box_2019, echo=FALSE, fig.align='left', fig.height=3.0, fig.cap='Ozone distribution by month (2019).'}
epa_data %>%
...
## Git Repository
https://github.com/hegao188/EDE_Spring2026
